...
import usCompanies from "./data/indianCompanies";

function IndianStocksPage({ setUSStocksTotal }) {
  ...
  useEffect(() => {
    const hasFetchedStocks = localStorage.getItem("showStocks");
    if (hasFetchedStocks) {
      setShowStocks(true);
      fetchStocks();
    }
  ...
  const handleFetchStocks = () => {
    setShowStocks(true); // Show stocks when button is clicked
    localStorage.setItem('showStocks', 'true'); // Store in localStorage
  ...
  const handleModifyStock = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/stocks/indian/${stockToModify.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteStock = async (id) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:3001/api/stocks/indian/${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        fetchStocks();
    ...
    <div className="container">
      ...
      {!showStocks ? (
        <div className="track-investments-container">
          <p className="track-investments-message">Do you want to fetch your Indian investments?</p>
